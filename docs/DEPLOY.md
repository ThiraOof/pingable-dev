...
> ขั้นที่ทำบน VM ได้อัตโนมัติ (ลง gns3server, bind `0.0.0.0`, เช็ค KVM, ทำเป็น
> systemd service) รวมไว้ใน [`scripts/deploy/enable-gns3.sh`](../scripts/deploy/enable-gns3.sh)
> — `bash scripts/deploy/enable-gns3.sh` บน VM แล้วทำตามขั้น A–D ที่มันพิมพ์ท้ายสุด.
> สรุปภาพรวมด้านล่าง:
