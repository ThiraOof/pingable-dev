...
> machine-type เป็น `n2-standard-2`). เปิด lab ทีหลังให้รัน GNS3 บน VM นี้แล้วตั้ง
> `GNS3_HOST=http://localhost`.
...
1. เปลี่ยน VM เป็น `n2-standard-2`+ (ต้องการ RAM สำหรับ VyOS VMs)
2. ลง GNS3 server บน VM เดียวกัน, register VyOS appliance, จด `template_id`
3. ใน `.env.production` ตั้ง `GNS3_HOST=http://localhost`, `GNS3_PORT=3080`,
   `GNS3_VYOS_TEMPLATE=<uuid>`, แล้ว seed ใหม่ + `up -d`
4. เบราว์เซอร์เข้า GNS3 ผ่าน proxy ของแอป — **ไม่ต้องเปิด port GNS3 ออกเน็ต**
