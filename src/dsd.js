/* ════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════ */
import { svgIcon, esc, mascot, mascotMark, TYPE_ICON, TYPE_LABEL } from './public/js/components/shared.js';
...
const T = {
  ...
  'png-modules': (a, data) => {
    ...
    const empty = `<div class="empty-state"><div class="empty-state-icon">${svgIcon('layers', 28)}</div><p>คอร์สนี้กำลังจัดเตรียมเนื้อหา — กลับมาดูใหม่เร็วๆ นี้</p></div>`;
  ...
  'png-duel': (a) => {
    const waiting = a.state === 'open';
    const invite = waiting
      ? `<div class="duel-invite"><p>ส่งลิงก์นี้ให้คู่แข่งเพื่อเริ่มดวล:</p><div class="duel-invite-row"><input id="duelInvite" readonly value="" class="duel-invite-input"><button id="btnCopyInvite" class="btn btn-secondary btn-sm">คัดลอก</button></div><p class="duel-waiting" id="duelWaiting">⏳ กำลังรอคู่แข่งเข้าร่วม…</p></div>`
      : '';
  ...
  'png-exam': (a) => {
    const n = +(a.number || 1), total = +(a.total || 1);
    const dots = Array.from({ length: total }, (_, i) => `<span class="exam-dot${i < n - 1 ? ' done' : i === n - 1 ? ' current' : ''}"></span>`).join('');
    const html = `<div class="exam-layout"><header class="exam-bar"><div class="exam-progress"><span class="exam-count">ข้อ ${n}/${total}</span><span class="exam-dots">${dots}</span></div><div class="exam-timer" id="examTimer">--:--</div></header><div class="exam-main"><div class="exam-lab-head"><span class="exam-lab-title">${svgIcon('flask', 16)} ${esc(a['lab-title'])}</span><span class="exam-status" id="examStatus">กำลังเตรียม Lab…</span></div><div class="gns3-wrapper"><div id="examLoading" class="lab-loading"><div class="provision"><div class="provision-title">กำลังเตรียมข้อสอบ</div><div class="provision-sub" id="examSub">โหลด topology…</div></div></div><iframe id="examFrame" class="gns3-frame"${a['gns3-url'] ? ` src="${esc(a['gns3-url'])}"` : ''} allowfullscreen></iframe></div></div><footer class="exam-controls"><button id="btnExamSkip" class="btn btn-ghost">ข้ามข้อนี้</button><button id="btnExamAbandon" class="btn btn-danger">ยอมแพ้ &amp; จบสอบ</button><button id="btnExamGrade" class="btn btn-primary" disabled>ตรวจ &amp; ไปข้อถัดไป</button></footer></div>`;
    return { host: ':host{display:block;height:calc(100dvh - var(--nav-h))}', html };
  },

  'png-lab': (a, data) => {
    const cid = a['course-id'], gradeCount = +(a['grade-count'] || 0);
    const objectives = (data && data.objectives) || [], hintCount = +((data && data.hintCount) || 0);
    const sc = data && data.scenario;
    const PRIO = { low: 'ต่ำ', medium: 'ปานกลาง', high: 'ด่วน' };
    const ticket = (sc && sc.body)
      ? `<div class="sidebar-section"><div class="lab-ticket${sc.priority ? ` prio-${esc(sc.priority)}` : ''}"><div class="ticket-head"><span class="ticket-tag">${svgIcon('activity', 13)} ใบแจ้งปัญหา</span>${sc.priority ? `<span class="ticket-prio">${PRIO[sc.priority] || esc(sc.priority)}</span>` : ''}</div>${sc.from ? `<div class="ticket-from">ผู้แจ้ง: ${esc(sc.from)}</div>` : ''}<p class="ticket-body">${esc(sc.body).replace(/\n/g, '<br>')}</p></div></div>`
      : '';
    const objList = objectives.length
      ...
      : '';
    // เนื้อคำใบ้ไม่อยู่ใน HTML — ปุ่มแต่ละใบยิง POST /hint/:idx ซึ่งบันทึก
    // การใช้ฝั่ง server (เปิดใบแรก = สละโบนัส no-hint, ใบถัดไปหัก XP ของแล็บ)
    const hintList = hintCount
      ? `<div class="sidebar-section"><h3>คำใบ้</h3><p class="hint-cost-note">${gradeCount ? 'เปิดคำใบ้ได้เสมอ แต่จะลดโบนัส XP ของแล็บนี้' : 'โหมดเล่นอิสระ — เปิดได้ไม่มีผลอะไร'}</p>${Array.from({ length: hintCount }, (_, i) => `<div class="hint-item hint-locked" id="hint-${i}" data-idx="${i}"><button type="button" class="hint-unlock">${svgIcon('arrow-right', 14)} เปิดคำใบ้ที่ ${i + 1}</button><p class="hint-text" hidden></p></div>`).join('')}</div>`
      : '';
    const gradeBtn = gradeCount
      ? `<button id="btnGrade" class="btn btn-primary btn-block" disabled title="กรุณารออุปกรณ์บูตให้เสร็จก่อน">${svgIcon('badge-check', 18)} ตรวจคำตอบ (${gradeCount} ข้อ)</button>`
      : `<div class="sandbox-note">${svgIcon('sparkles', 16)} โหมดเล่นอิสระ — Lab นี้ไม่มีการตรวจคะแนน ทดลองได้เต็มที่</div>`;
    const historySection = `<div class="sidebar-section" id="labHistoryWrap" hidden><h3>${svgIcon('clock', 16)} ประวัติการตรวจ</h3><div id="labHistory" class="lab-history"></div></div>`;
    // จดโน้ตด่วนระหว่างทำ lab — ส่งเข้า /notes โดยไม่ต้องสลับแท็บ
    const noteSection = `<div class="sidebar-section"><h3>${svgIcon('sparkles', 16)} จดโน้ต</h3><textarea id="labNote" class="lab-note-input" rows="3" placeholder="จดคำสั่งหรือเคล็ดลับที่เพิ่งเจอ..."></textarea><button id="btnSaveNote" class="btn btn-secondary btn-block btn-sm">บันทึกลงสมุดโน้ต</button><span id="labNoteMsg" class="lab-note-msg"></span></div>`;
    const html = `<div class="lab-layout"><div id="labScrim" class="lab-scrim" hidden></div><aside class="lab-sidebar" id="labSidebar"><div class="lab-sidebar-header"><a href="/courses/${esc(cid)}" class="back-link">${svgIcon('arrow-left', 16)} ${esc(a['course-title'])}</a><h2>${svgIcon('flask', 22)} ${esc(a.title)}</h2>${a.mode === 'troubleshoot' ? `<span class="lab-mode-chip troubleshoot">🔧 โหมดซ่อม — ระบบพังมาแล้ว หาให้เจอ</span>` : ''}${'is-boss' in a ? `<span class="lab-mode-chip boss">👹 BOSS LAB</span>` : ''}<p>${esc(a.desc)}</p></div>${ticket}${objList}${hintList}${noteSection}${historySection}<div class="lab-controls">${gradeBtn}<button id="btnStop" class="btn btn-danger btn-block">${svgIcon('stop', 18)} หยุด Lab &amp; คืนทรัพยากร</button></div></aside><div id="gradeModal" class="grade-modal-overlay" hidden><div class="grade-modal"><div class="grade-modal-header"><h3>ผลการตรวจคำตอบ</h3><button id="btnCloseModal" class="grade-modal-close" aria-label="ปิด">${svgIcon('x', 22)}</button></div><div class="grade-score-hero"><png-mascot id="gradeMascot" class="grade-mascot" state="idle" size="84"></png-mascot><div class="score-ring" id="scoreRing"><svg width="96" height="96"><circle class="track" cx="48" cy="48" r="42"/><circle class="fill" id="scoreRingFill" cx="48" cy="48" r="42"/></svg><div class="center"><div><span class="pct" id="scorePct">0%</span><small>คะแนน</small></div></div></div><div class="grade-score-meta"><div class="grade-banner" id="gradeBanner"></div><p id="gradeScoreText"></p></div></div><ul class="grade-result-list" id="gradeResultList"></ul><div class="mentor-box" id="mentorBox" hidden><button id="btnMentor" class="btn btn-secondary btn-block">${svgIcon('sparkles', 16)} ขอคำแนะนำจากพี่เลี้ยง AI</button><div class="mentor-reply" id="mentorReply" hidden></div></div></div></div><div class="lab-main"><div class="lab-toolbar"><button id="btnToggleSidebar" class="lab-sidebar-toggle">${svgIcon('book-open', 16)} คำแนะนำ</button><span id="labStatus" class="lab-status status-loading">กำลังเตรียม Lab...</span></div><div class="gns3-wrapper"><div id="labLoading" class="lab-loading"><div class="provision" id="provisionCard"><png-mascot class="provision-mascot" state="thinking" size="76"></png-mascot><div class="provision-title">กำลังเตรียม Lab</div><div class="provision-sub" id="provisionSub">initializing workspace…</div><ul class="provision-steps" id="provisionSteps"></ul></div><div class="lab-stopped" id="stoppedCard" hidden><png-mascot state="idle" size="76"></png-mascot><p id="stoppedText">Lab หยุดทำงานแล้ว — คืนทรัพยากรเรียบร้อย</p><button class="btn btn-primary" id="btnRestart">${svgIcon('play', 18)} เริ่ม Lab ใหม่</button></div></div><iframe id="gns3Frame" class="gns3-frame" allowfullscreen></iframe></div></div></div>`;
    return { host: ':host{display:block;height:calc(100dvh - var(--nav-h))}', html };
  },
};
