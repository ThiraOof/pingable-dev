      ...
      lessons: [
        ...
        },
        {
          ...
          hints: [
            ...
            'สร้าง ruleset ขาออก: `set firewall ipv4 name LAN-WAN default-action drop` · `... rule 10 action accept` · `... rule 10 state new`',
            'สร้าง ruleset ขากลับ (stateful): `set firewall ipv4 name WAN-LAN rule 10 action accept` · `... rule 10 state established` · `... rule 10 state related` · `... default-action drop`',
            'ประกาศโซนและผูก: `set firewall zone LAN member interface eth1` · `set firewall zone WAN member interface eth2` · `set firewall zone WAN from LAN firewall name LAN-WAN` · `set firewall zone LAN from WAN firewall name WAN-LAN`',
          ...
          gradingChecks: [
            { description: 'R1 ประกาศ security zone', node: 'R1', command: 'show configuration commands | match "firewall zone"', expect: 'zone', points: 3,
              failHint: 'ยังไม่มีโซน — เริ่มจาก `set firewall zone LAN member interface eth1` และ `set firewall zone WAN member interface eth2` (เมื่อ interface เข้าโซนแล้ว ทุกอย่างที่ไม่มีกฎอนุญาตจะถูก drop โดยปริยาย)' },
            { description: 'R1 มีกฎ stateful (state established) ฝั่งขากลับ', node: 'R1', command: 'show configuration commands | match "state established"', expect: 'established', points: 3,
              failHint: 'นี่คือหัวใจ stateful: ruleset ทิศ WAN→LAN ต้องมี `rule 10 state established` (และ related) ไม่งั้นของที่ขอออกไปจะตอบกลับเข้ามาไม่ได้' },
      ...
      lessons: [
        ...
        },
        {
          ...
          hints: [
            ...
            'IKE/ESP (ตั้งให้เหมือนกันสองฝั่ง): `set vpn ipsec ike-group IKE key-exchange ikev2` · `set vpn ipsec ike-group IKE proposal 1 encryption aes256` · `... hash sha256` · `... dh-group 14` และ `set vpn ipsec esp-group ESP proposal 1 encryption aes256` · `... hash sha256`',
            ...
            'peer R1 (VyOS 1.5: ชื่อ peer เป็นตัวอักษร ไม่ใช่ IP): `set vpn ipsec site-to-site peer PEER1 authentication mode pre-shared-secret` · `... authentication local-id 10.0.12.1` · `... authentication remote-id 10.0.12.2` · `... remote-address 10.0.12.2` · `... local-address 10.0.12.1` · `... ike-group IKE` · `... default-esp-group ESP` · `... tunnel 1 local prefix 192.168.1.0/24` · `... tunnel 1 remote prefix 192.168.2.0/24`',
            'R2 ทำกลับด้าน (remote-address 10.0.12.1, local-address 10.0.12.2, local-id/remote-id สลับกัน, tunnel local 192.168.2.0/24, remote 192.168.1.0/24, PSK secret เดียวกัน) แล้วตรวจ `show vpn ipsec sa` ต้องเห็น up',
          ],
          topology: {
          ...
          gradingChecks: [
            { description: 'R1 ตั้งค่า site-to-site peer แบบ pre-shared-secret', node: 'R1', command: 'show configuration commands | match "pre-shared-secret"', expect: 'pre-shared-secret', points: 2,
              failHint: 'ยังไม่มี peer — `set vpn ipsec site-to-site peer PEER1 authentication mode pre-shared-secret` พร้อม remote-address, local-address, ike-group, default-esp-group และ tunnel 1 local/remote prefix ให้ครบ (VyOS 1.5 ชื่อ peer เป็นตัวอักษร)' },
