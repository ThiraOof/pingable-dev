  ...
  modules: [
    {
      ...
      lessons: [
        ...
        },
        {
          ...
          scenario: {
            from: 'พี่กล้า Network Engineer รุ่นพี่',
            priority: 'medium',
            body: 'ลูกค้า SME รายหนึ่งเช่า IP ได้แค่วง /26 มาใช้ทั้งสำนักงาน น้องช่วยวางเครื่องสามตัวแรกลงในวง 192.168.10.0/26 ให้หน่อย จำไว้นะ mask ไม่ใช่ /24 ที่คุ้นมือ — ถ้าใส่ mask ผิดตัวเดียว ทั้งวงจะมองกันไม่เห็นแบบงง ๆ เลย',
          },
          objectives: [
          ...
          gradingChecks: [
            { description: 'PC1 ตั้งค่า 192.168.10.11/26', node: 'PC1', command: 'show ip', expect: '192\\.168\\.10\\.11/26', points: 2,
              failHint: 'PC1 ต้องเป็น /26 ไม่ใช่ /24 — ใช้ `ip 192.168.10.11 255.255.255.192` แล้วดู `show ip` ว่าขึ้น /26' },
            { description: 'PC2 ตั้งค่า 192.168.10.22/26', node: 'PC2', command: 'show ip', expect: '192\\.168\\.10\\.22/26', points: 2,
              failHint: 'PC2 ยังไม่ใช่ 192.168.10.22/26 — mask /26 คือ 255.255.255.192 (จุดที่พลาดบ่อยที่สุดของแล็บนี้)' },
            { description: 'PC3 ตั้งค่า 192.168.10.33/26', node: 'PC3', command: 'show ip', expect: '192\\.168\\.10\\.33/26', points: 2,
              failHint: 'PC3 ยังไม่ใช่ 192.168.10.33/26 — เช็คว่า mask ตรงกับอีกสองเครื่อง ถ้าเครื่องเดียวใช้ mask ต่างกันจะมองกันคนละ network' },
            { description: 'PC1 ping PC2 ได้', node: 'PC1', command: 'ping 192.168.10.22', expect: 'bytes from 192\\.168\\.10\\.22', points: 2,
              failHint: 'ping ไม่ถึง PC2 — ทั้งสองเครื่องต้องอยู่ block เดียวกันของ /26 (.1–.62) และ mask 255.255.255.192 เหมือนกัน' },
            { description: 'PC1 ping PC3 ได้', node: 'PC1', command: 'ping 192.168.10.33', expect: 'bytes from 192\\.168\\.10\\.33', points: 2,
              failHint: 'ping ไม่ถึง PC3 — ไล่เช็ค `show ip` ทีละเครื่อง: IP อยู่ช่วง .1–.62, mask /26, host part ไม่ซ้ำ' },
          ],
        },
        {
          type: 'quiz',
      ...
      lessons: [
        ...
        },
        {
          ...
          scenario: {
            from: 'คุณแพร ฝ่ายขาย',
            priority: 'medium',
            body: 'เครื่องในห้องขายแชร์ไฟล์หากันได้ปกติ แต่พอจะเข้าเว็บระบบ CRM ที่สำนักงานใหญ่กลับขึ้น "ติดต่อไม่ได้" ทุกเครื่องเลยค่ะ พี่ IT เก่าเคยพึมพำว่า "ลืมใส่เกตเวย์รึเปล่า" แล้วก็ลาออกไปแล้ว... ฝากตั้งให้ถูกด้วยนะคะ',
          },
          objectives: [
          ...
          gradingChecks: [
            { description: 'PC1 มี IP 192.168.20.10/24', node: 'PC1', command: 'show ip', expect: '192\\.168\\.20\\.10/24', points: 2,
              failHint: 'PC1 ยังไม่ได้ IP — VPCS ตั้ง IP กับ gateway ในคำสั่งเดียว: `ip 192.168.20.10 255.255.255.0 192.168.20.1`' },
            { description: 'PC1 ตั้ง gateway 192.168.20.1', node: 'PC1', command: 'show ip', expect: 'GATEWAY\\s*:\\s*192\\.168\\.20\\.1', points: 3,
              failHint: 'ใน `show ip` ของ PC1 บรรทัด GATEWAY ยังว่างหรือไม่ใช่ 192.168.20.1 — ใส่ gateway เป็นพารามิเตอร์ตัวที่สามของคำสั่ง `ip`' },
            { description: 'PC2 มี IP 192.168.20.20/24', node: 'PC2', command: 'show ip', expect: '192\\.168\\.20\\.20/24', points: 2,
              failHint: 'PC2 ยังไม่ได้ IP — ใช้ `ip 192.168.20.20 255.255.255.0 192.168.20.1`' },
            { description: 'PC2 ตั้ง gateway 192.168.20.1', node: 'PC2', command: 'show ip', expect: 'GATEWAY\\s*:\\s*192\\.168\\.20\\.1', points: 3,
              failHint: 'gateway ของ PC2 ยังไม่ถูก — สังเกตว่าการพิมพ์ `ip` ใหม่โดยไม่ใส่พารามิเตอร์ที่สามจะลบ gateway เดิมทิ้ง ต้องใส่ครบทั้งสามค่า' },
            { description: 'PC1 ping PC2 ภายใน subnet ได้', node: 'PC1', command: 'ping 192.168.20.20', expect: 'bytes from 192\\.168\\.20\\.20', points: 2,
              failHint: 'ping ภายในวงไม่ผ่าน — gateway ไม่เกี่ยวกับ ping ในวงเดียวกัน เช็คว่า IP/mask ของทั้งคู่อยู่ 192.168.20.x/24 ทั้งสองเครื่อง' },
          ],
        },
        {
          type: 'quiz',
