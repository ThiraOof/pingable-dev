    ...
    // ── Module 2 ───────────────────────────────────────────────
    {
      ...
      lessons: [
        ...
        },
        {
          ...
          scenario: {
            from: 'คุณนัท ฝ่ายธุรการ',
            priority: 'medium',
            body: 'ออฟฟิศเพิ่งซื้อคอมใหม่สองเครื่องมาตั้งโต๊ะหน้า-หลัง ช่างเดินสายแลนเสร็จแล้วแต่สองเครื่องยังมองไม่เห็นกันเลยค่ะ แชร์ไฟล์ก็ไม่ได้ พี่ IT บอกว่า "ยังไม่ได้ใส่ IP" — ฝากตั้งค่าให้หน่อยนะคะ งานต้องส่งบ่ายนี้',
          },
          objectives: [
          ...
          gradingChecks: [
            { description: 'PC1 มี IP 192.168.1.1/24', node: 'PC1', command: 'show ip', expect: '192\\.168\\.1\\.1', points: 2,
              failHint: 'PC1 ยังไม่มี IP 192.168.1.1 — เปิด console ของ PC1 แล้วพิมพ์ `ip 192.168.1.1 255.255.255.0` จากนั้นเช็คด้วย `show ip`' },
            { description: 'PC2 มี IP 192.168.1.2/24', node: 'PC2', command: 'show ip', expect: '192\\.168\\.1\\.2', points: 2,
              failHint: 'PC2 ยังไม่มี IP 192.168.1.2 — เปิด console ของ PC2 แล้วพิมพ์ `ip 192.168.1.2 255.255.255.0`' },
            { description: 'PC1 ping PC2 ได้สำเร็จ', node: 'PC1', command: 'ping 192.168.1.2', expect: 'bytes from 192\\.168\\.1\\.2', points: 3,
              failHint: 'ping จาก PC1 ไป PC2 ยังไม่ตอบ — ใช้ `show ip` บนทั้งสองเครื่อง เทียบว่า IP อยู่วง 192.168.1.x และ mask /24 ตรงกันทั้งคู่หรือไม่' },
            { description: 'PC2 ping PC1 ได้สำเร็จ', node: 'PC2', command: 'ping 192.168.1.1', expect: 'bytes from 192\\.168\\.1\\.1', points: 3,
              failHint: 'ping ฝั่งขากลับยังไม่ผ่าน — ถ้า PC1→PC2 ได้แต่ PC2→PC1 ไม่ได้ มักเป็น IP ของ PC1 พิมพ์ผิดหรือ mask ไม่ตรง ลอง `show ip` บน PC1 อีกครั้ง' },
          ],
        },
        {
          type: 'quiz',
      ...
      lessons: [
        ...
        },
        {
          ...
          scenario: {
            ...
            priority: 'low',
            body: 'เราจะแยกเครื่องฝ่ายบัญชีกับฝ่ายขายออกเป็นคนละวงตามนโยบายใหม่ น้องลองจัดวง 192.168.1.x ให้บัญชี และ 10.0.0.x ให้ฝ่ายขายดู เสร็จแล้วช่วยพิสูจน์ให้พี่ดูหน่อยว่าในวงเดียวกันคุยกันได้ แต่ข้ามวง "ต้องคุยไม่ได้" เพราะเรายังไม่มี router นะ',
          },
          objectives: [
          ...
          gradingChecks: [
            { description: 'PC1 มี IP 192.168.1.1/24', node: 'PC1', command: 'show ip', expect: '192\\.168\\.1\\.1', points: 1,
              failHint: 'PC1 ยังไม่ได้ IP ที่กำหนด — พิมพ์ `ip 192.168.1.1 255.255.255.0` ใน console ของ PC1' },
            { description: 'PC2 มี IP 192.168.1.2/24', node: 'PC2', command: 'show ip', expect: '192\\.168\\.1\\.2', points: 1,
              failHint: 'PC2 ยังไม่ได้ IP ที่กำหนด — พิมพ์ `ip 192.168.1.2 255.255.255.0` ใน console ของ PC2' },
            { description: 'PC3 มี IP 10.0.0.1/24', node: 'PC3', command: 'show ip', expect: '10\\.0\\.0\\.1', points: 1,
              failHint: 'PC3 อยู่ฝั่ง SW2 ต้องใช้วง 10.0.0.x — พิมพ์ `ip 10.0.0.1 255.255.255.0` ใน console ของ PC3' },
            { description: 'PC4 มี IP 10.0.0.2/24', node: 'PC4', command: 'show ip', expect: '10\\.0\\.0\\.2', points: 1,
              failHint: 'PC4 อยู่ฝั่ง SW2 ต้องใช้วง 10.0.0.x — พิมพ์ `ip 10.0.0.2 255.255.255.0` ใน console ของ PC4' },
            { description: 'PC1 ping PC2 ได้ (subnet เดียวกัน)', node: 'PC1', command: 'ping 192.168.1.2', expect: 'bytes from 192\\.168\\.1\\.2', points: 3,
              failHint: 'PC1 กับ PC2 อยู่ switch เดียวกันแต่ยัง ping ไม่ถึง — เช็ค `show ip` ทั้งคู่ว่า network part เป็น 192.168.1 เหมือนกันและ host part ไม่ซ้ำกัน' },
            { description: 'PC3 ping PC4 ได้ (subnet เดียวกัน)', node: 'PC3', command: 'ping 10.0.0.2', expect: 'bytes from 10\\.0\\.0\\.2', points: 3,
              failHint: 'PC3 กับ PC4 ควร ping กันได้ในวง 10.0.0.x — เช็คว่าทั้งคู่ใช้ mask 255.255.255.0 และไม่เผลอใส่วง 192.168.1.x' },
          ],
        },
        {
          type: 'quiz',
