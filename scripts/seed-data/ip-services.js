          ...
          sections: [
            ...
            },
            {
              ...
              body: [
                ...
                '',
                '```',
                'set system syslog host 10.0.12.2 facility all level warning',
                '```',
                '',
      ...
      lessons: [
          ...
          sections: [
            ...
            },
            {
              ...
              body: [
                ...
                '',
                '```',
                'set traffic-policy shaper WAN-OUT bandwidth 10mbit',
                ...
                'set traffic-policy shaper WAN-OUT class 10 priority',
        ...
        },
        {
          ...
          hints: [
            ...
            'สร้าง shaper: `set traffic-policy shaper WAN-OUT bandwidth 10mbit` · `set traffic-policy shaper WAN-OUT default bandwidth 50%`',
            'คลาสเสียง: `set traffic-policy shaper WAN-OUT class 10 match VOICE ip dscp EF` · `set traffic-policy shaper WAN-OUT class 10 bandwidth 30%` · `set traffic-policy shaper WAN-OUT class 10 priority`',
            'ผูกเข้า interface: `set interfaces ethernet eth1 traffic-policy out WAN-OUT` แล้ว commit · ทดสอบ `ping 10.0.12.2` ต้องยังผ่าน (QoS ไม่ควรตัดการส่งต่อปกติ)',
          ],
          topology: {
            nodes: [ vyos('R1', -180, 0), vyos('R2', 180, 0) ],
            links: [ { node1: 'R1', port1: 1, node2: 'R2', port2: 1 } ],
          ...
          gradingChecks: [
            { description: 'R1 สร้าง traffic-policy แบบ shaper', node: 'R1', command: 'show configuration commands | match "traffic-policy shaper"', expect: 'shaper', points: 3,
              failHint: 'ยังไม่มี shaper — `set traffic-policy shaper WAN-OUT bandwidth 10mbit` พร้อม default bandwidth และคลาสสำหรับเสียง (match ... ip dscp EF)' },
            { description: 'R1 ผูก traffic-policy เข้ากับ interface ขาออก', node: 'R1', command: 'show configuration commands | match "traffic-policy out"', expect: 'traffic-policy out', points: 4,
              failHint: 'policy ลอยอยู่เฉย ๆ — ต้องผูกเข้า interface ทิศ out: `set interfaces ethernet eth1 traffic-policy out WAN-OUT` กฎถึงจะมีผลกับทราฟฟิกขาออกจริง' },
