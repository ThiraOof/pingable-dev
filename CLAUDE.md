...
1. `POST /lab/:courseId/:m/:l/start` → `labSessionService.startSession()` **atomically claims** the user's slot (`status: 'building'` doubles as a start lock — a concurrent start gets a 409 `LabBusyError`; a stale build is reclaimable after 5 min), tears down the user's previous project, then `gns3Service.buildLab()` creates the project/nodes/links and starts all nodes. `buildLab` deletes its own half-built project if it fails partway. The doc then stores `{ projectId, webUiUrl, nodes }` with `status: 'ready'`.
